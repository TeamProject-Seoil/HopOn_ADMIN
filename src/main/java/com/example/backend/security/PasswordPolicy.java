...
public final class PasswordPolicy {
    ...
    public static String validateAndReason(String pwd) {
        if (pwd == null)
            return "비밀번호가 비어 있습니다.";
        int len = pwd.length();
        if (len < 8 || len > 64)
            return "비밀번호는 8~64자로 입력하세요.";

        // (변경) 특수문자 허용 → 영문/숫자만 강제하던 검사 제거
        ...
        if (classes < 2)
            return "영문 대문자, 소문자, 숫자 중 2종류 이상을 섞어주세요.";

        // 3자리 연속 증가/감소 숫자/알파벳 금지
        if (hasSequentialAlphaOrDigit(pwd))
            return "연속된 문자/숫자열(3자리 이상)은 사용할 수 없습니다.";

        // 키보드 행 시퀀스 금지(qwerty/asdf/zxc 등)
        if (hasKeyboardSequence(pwd))
            return "키보드 상 연속된 문자열(3자리 이상)은 사용할 수 없습니다.";
